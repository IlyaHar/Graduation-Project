...
@section('content')
    <div class="d-flex flex-column align-items-center">
        ...
        <form action="{{ route('auth.register') }}" method="POST" class="w-25" novalidate>
            @csrf
            <input type="email" name="email" class="form-control mt-4" placeholder="Введите email" value="{{ old('email') }}">
            <input type="text" name="login" class="form-control  mt-3" placeholder="Введите логин"  value="{{ old('login') }}">
            <input type="password" name="password" class="form-control mt-3" placeholder="Введите пароль"  value="{{ old('password') }}">
            @include('blocks.messages')
            <button type="submit" class="mt-3 btn btn-danger">Зарегистрироваться</button>
        </form>
        <p class="mt-4">Есть аккаунт? Тогда вы можете <a href="{{ route('auth.index') }}" class="text-primary">авторизоваться</a></p>
        @endguest
        @auth()
            ...
            <form action="{{ route('link.store') }}" method="POST" class="w-25" novalidate>
                @csrf
                <input type="text" name="link" class="form-control mt-4" placeholder="Длинная ссылка" value="{{ old('link') }}">
                <input type="text" name="title" class="form-control  mt-3" placeholder="Короткое название"  value="{{ old('title') }}">
                @include('blocks.messages')
